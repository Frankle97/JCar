...
public class CarDao {
	public ArrayList<CarlistDto> listCarSearch(String search){
		...
		try {
			...
			while (rset.next()) {
				list.add(new CarlistDto(rset.getInt("no"), 
						rset.getInt("birth"), 
						rset.getInt("km"), 
						rset.getInt("price"), 
						rset.getString("country"), 
						rset.getString("maker"), 
						rset.getString("category"), 
						rset.getString("model"), 
						rset.getString("detail"), 
						rset.getString("color"), 
						rset.getString("fuel"), 
						rset.getString("mission"), 
						rset.getString("options"), 
						rset.getString("accident"), 
						rset.getString("seater"), 
						rset.getString("city"), 
	...
	public int addListCar(CarlistDto dto) {
		int res = 0;
		String sql = "insert into carlist (birth, "
				+ "km, price, country, model, detail, color, fuel, mission, options, accident, seater, city, writer, ip, image, content, maker, category) "
				+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		Connection conn = null; 
		PreparedStatement pstmt = null;
	...
	public CarlistDto detailCar(int no) {
		...
		try {
			...
			rset = pstmt.executeQuery();
			while(rset.next()) {
				dto = new CarlistDto(rset.getInt("no"), 
						rset.getInt("birth"), 
						rset.getInt("km"), 
						rset.getInt("price"), 
						rset.getString("country"), 
	...
	public int removeCar(int no) {
		...
		try {
			...
			if (conn!=null || pstmt != null || pstmt2 != null) { 
				try {
					conn.close();
				} catch (Exception e) {
					e.printStackTrace();
