...
public class UserChangeInfoAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		int res = new MemberDao().changeInfo(request.getParameter("id"), 
				request.getParameter("password"), 
				request.getParameter("email"), 
				request.getParameter("contact"), address);
		
		if (res > 0) {
			out.println("<script>alert('회원님의 정보 수정이 완료되었습니다.'); "
					+ "location.href='" + request.getContextPath() + "/myPageView.do"
							+ "?id=" + request.getParameter("id") + "'; </script>");
		} else {
			out.println("<script>alert('회원님의 정보 수정에 실패하였습니다. 관리자에게 문의해주세요.'); "
					+ "location.href='" + request.getContextPath() + "/changeMyInfo.do"
							+ "?id=" + request.getParameter("id") + "'; </script>");
		}
	}
